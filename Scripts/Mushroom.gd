...
func _physics_process(delta):
	...
	if player:
		if global.mushroom_enemy_alive == true:
	...
	if player_inattack_zone == true and global.mushroom_enemy_alive == true:
		animation.play("mushroom_attack")
	
	if motion.x > 0:
		$mushroom_sprite.flip_h = false
	if motion.x < 0:
...
func deal_with_damage_1():
	if player_inattack_zone == true and global.player_attack_1 == true:
		if can_take_damage == true:
			...
			if health <= 0:
				health = 0
				global.mushroom_enemy_alive = false
				animation.play("mushroom_death")
				$mushroom_death.start()
...
func _on_mushroom_death_timeout():
	self.queue_free()
...
func _on_enemy_alive_timer_timeout():
	global.mushroom_enemy_alive = true
