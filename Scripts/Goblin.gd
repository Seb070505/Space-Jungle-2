...
func _physics_process(delta):
	...
	if player:
		if global.enemy_alive == true:
			motion = position.direction_to(player.position) * speed
			if player_inattack_zone == false:
				animation.play("goblin_run")

	if player == null:
		animation.play("goblin_idle")
	
	if player_inattack_zone == true and global.enemy_alive == true:
		animation.play("goblin_attack")
	
	if motion.x > 0:
		$GoblinSprite.flip_h = false
	if motion.x < 0:
		$GoblinSprite.flip_h = true
